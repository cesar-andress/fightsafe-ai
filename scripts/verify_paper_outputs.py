...
def verify_iswa(results_csv: Path, reference_csv: Path) -> list[str]:
    errors: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--paper", choices=("fusion", "iswa", "sports", "all"), default="all")
    ...
    if args.paper in ("iswa", "all"):
        ...
        ref_csv = repro_root / "iswa2026/reference/tapko_results.csv"
        if not repro_csv.is_file():
            # Reference-only check when video not re-run
            if ref_csv.is_file():
                print("iswa: using reference snapshot only (no fresh eval outputs).")
            else:
                all_errors.append("iswa: no reproduced or reference tapko_results.csv")
        else:
            all_errors.extend(verify_iswa(repro_csv, ref_csv))
