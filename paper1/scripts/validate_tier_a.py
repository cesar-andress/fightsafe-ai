#!/usr/bin/env python3.12
"""Validate Tier A from inside the staging package (third-party simulation)."""
from __future__ import annotations

import hashlib
import json
import re
import shutil
import subprocess
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
VAL = ROOT / "validation"
VAL.mkdir(exist_ok=True)


def log(name: str, text: str) -> None:
    (VAL / name).write_text(text, encoding="utf-8")


def sha256(path: Path) -> str:
    h = hashlib.sha256()
    with path.open("rb") as f:
        for chunk in iter(lambda: f.read(1 << 20), b""):
            h.update(chunk)
    return h.hexdigest()


def main() -> int:
    results: dict[str, str] = {}
    failed = False

    # 1. Environment / Python
    results["python"] = sys.version.split()[0]
    if not sys.version.startswith("3.12"):
        results["python_check"] = "WARN: expected 3.12"
    else:
        results["python_check"] = "PASS"

    # 2. Checksums
    cp = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "verify_checksums.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    log("checksums.txt", cp.stdout + cp.stderr)
    results["checksums"] = "PASS" if cp.returncode == 0 else "FAIL"
    if cp.returncode != 0:
        failed = True

    # 3. Import aggregation
    env = dict(**{k: v for k, v in __import__("os").environ.items()})
    env["PYTHONPATH"] = str(ROOT / "src")
    imp = subprocess.run(
        [
            sys.executable,
            "-c",
            "from fightsafe_ai.evaluation.aggregation_schemes import SCHEME_ORDER, compute_pre_interaction_with_scheme;"
            "assert set(['equal','weighted','max']).issubset(SCHEME_ORDER); print('OK', SCHEME_ORDER)",
        ],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
    )
    log("import_aggregation.txt", imp.stdout + imp.stderr)
    results["import_aggregation"] = "PASS" if imp.returncode == 0 else "FAIL"
    if imp.returncode != 0:
        failed = True

    # Hash match
    agg = ROOT / "src/fightsafe_ai/evaluation/aggregation_schemes.py"
    expect = "4d19f0f69238c9afdf365b6baeae26986a2d7086fb2b24170f5b2ce5b6209281"
    got = sha256(agg)
    results["aggregation_hash"] = "PASS" if got == expect else f"FAIL {got}"
    if got != expect:
        failed = True

    # 4. Canonical CSV parse + numerical integrity
    import pandas as pd

    summary = pd.read_csv(ROOT / "results/run_20260730_005150/experiment_summary.csv")
    paired = pd.read_csv(ROOT / "results/run_20260730_005150/paired_comparisons.csv")
    dropout = pd.read_csv(ROOT / "results/run_20260730_005150/dropout_results.csv")
    numbers = json.loads((ROOT / "tables/numbers.json").read_text(encoding="utf-8"))

    def get_micro(method: str) -> float:
        row = summary[(summary.experiment == "aggregation") & (summary.method == method)].iloc[0]
        return float(row.micro_f1)

    checks = {
        "equal_micro_f1": abs(get_micro("equal") - numbers["micro_f1_equal"]) < 1e-12,
        "weighted_micro_f1": abs(get_micro("weighted") - numbers["micro_f1_weighted"]) < 1e-12,
        "max_micro_f1": abs(get_micro("max") - numbers["micro_f1_max"]) < 1e-12,
        "macro_int_off": abs(
            float(
                summary[
                    (summary.experiment == "interactions") & (summary.interactions == "off")
                ].iloc[0].macro_f1
            )
            - numbers["macro_f1_int_off"]
        )
        < 1e-12,
        "macro_int_on": abs(
            float(
                summary[
                    (summary.experiment == "interactions") & (summary.interactions == "on")
                ].iloc[0].macro_f1
            )
            - numbers["macro_f1_int_on"]
        )
        < 1e-12,
        "approx_equal_0.502": abs(get_micro("equal") - 0.502) < 5e-4,
        "approx_weighted_0.502": abs(get_micro("weighted") - 0.502) < 5e-4,
        "approx_max_0.178": abs(get_micro("max") - 0.178) < 5e-4,
    }
    mm = dropout[dropout.video_id == "__MICRO_MACRO__"]
    ex = float(mm[(mm.p == 0.5) & (mm["mode"] == "explicit_alpha0")].micro_f1.mean())
    nv = float(mm[(mm.p == 0.5) & (mm["mode"] == "naive_zero")].micro_f1.mean())
    checks["dropout_explicit_p05"] = abs(ex - numbers["dropout_p05_explicit"]) < 1e-12
    checks["dropout_naive_p05"] = abs(nv - numbers["dropout_p05_naive"]) < 1e-12
    checks["approx_explicit_0.509"] = abs(ex - 0.509) < 5e-4
    checks["approx_naive_0.471"] = abs(nv - 0.471) < 5e-4
    p_int = float(
        paired.loc[paired.comparison == "weighted_intON_minus_intOFF", "permutation_pvalue_twosided"].iloc[0]
    )
    checks["paired_int_p"] = abs(p_int - numbers["paired_int_p"]) < 1e-12
    log("numerical_checks.json", json.dumps({k: bool(v) for k, v in checks.items()}, indent=2))
    if not all(checks.values()):
        results["numerical_integrity"] = "FAIL"
        failed = True
    else:
        results["numerical_integrity"] = "PASS"

    # 5–7. Regenerate figures/tables and compare key numbers in regenerated tables
    before_figs = {p.name: sha256(p) for p in (ROOT / "figures").glob("fig_*.pdf")}
    before_tabs = {
        p.name: p.read_text(encoding="utf-8")
        for p in list((ROOT / "tables").glob("tab_*.tex"))
        + list((ROOT / "supplementary").glob("tab_si_*.tex"))
    }
    gen = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "generate_assets.py")],
        cwd=ROOT,
        capture_output=True,
        text=True,
    )
    log("generate_assets.txt", gen.stdout + gen.stderr)
    results["generate_assets"] = "PASS" if gen.returncode == 0 else "FAIL"
    if gen.returncode != 0:
        failed = True
    else:
        after_nums = json.loads((ROOT / "tables/numbers.json").read_text(encoding="utf-8"))
        num_ok = all(abs(float(after_nums[k]) - float(numbers[k])) < 1e-12 for k in numbers if k != "canon_relative")
        results["regenerated_numbers"] = "PASS" if num_ok else "FAIL"
        if not num_ok:
            failed = True
        # regenerated tex should contain same numeric tokens for key metrics
        agg_tex = (ROOT / "tables/tab_aggregation.tex").read_text(encoding="utf-8")
        for token in ("0.502", "0.178"):
            if token not in agg_tex:
                results["regenerated_tables"] = f"FAIL missing {token}"
                failed = True
                break
        else:
            results["regenerated_tables"] = "PASS"
        fig_ok = all((ROOT / "figures" / n).is_file() for n in before_figs)
        results["regenerated_figures"] = "PASS" if fig_ok else "FAIL"
        if not fig_ok:
            failed = True

    # 8. Unit tests
    test = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/unit/test_aggregation_schemes.py", "-q"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        env=env,
    )
    log("pytest_aggregation.txt", test.stdout + test.stderr)
    results["unit_tests"] = "PASS" if test.returncode == 0 else "FAIL"
    if test.returncode != 0:
        failed = True

    # 9. Absolute path scan (Tier A; allow README exclusion list mentions)
    abs_hits = []
    skip_suffixes = {".pdf", ".png", ".xlsx", ".ods", ".parquet", ".pkl", ".pyc"}
    for p in ROOT.rglob("*"):
        if not p.is_file():
            continue
        rel = p.relative_to(ROOT).as_posix()
        if "__pycache__" in rel or rel.startswith("validation/") or rel.startswith("optional_tier_b/"):
            continue
        if Path(rel).suffix.lower() in skip_suffixes:
            continue
        if rel in {
            "scripts/validate_tier_a.py",
            "environment/pip_freeze.txt",
            "environment/pip_freeze.ORIGINAL_NOTE.md",
            "RELEASE_STATUS.md",
        }:
            continue
        try:
            text = p.read_text(encoding="utf-8", errors="ignore")
        except Exception:
            continue
        # Active absolute path usage (not scrubbed placeholders / comments about exclusions)
        if re.search(r"(?<!REDACTED_LOCAL_EDITABLE: -e )(?<!<)/home/cesar/[A-Za-z0-9_./-]+", text):
            if "/home/cesar" in text and "REDACTED" not in text and "<LOCAL_HOME" not in text and "<NOT_REDISTRIBUTED>" not in text:
                abs_hits.append(rel)
    log("absolute_path_hits.txt", "\n".join(abs_hits) if abs_hits else "NONE\n")
    results["absolute_paths"] = "PASS" if not abs_hits else f"FAIL {abs_hits[:10]}"
    if abs_hits:
        failed = True

    # 10. Metadata residue (obsolete journal positioning as current identity)
    meta_hits = []
    for rel in ("CITATION.cff", "zenodo.json"):
        text = (ROOT / rel).read_text(encoding="utf-8")
        if re.search(r"\bISWA\b|Information Fusion", text, re.I):
            meta_hits.append(rel)
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    # README may list excluded ISWA/IF material; fail only if framed as current deliverable
    if re.search(r"for the ISWA|ISWA 2026 traceability|companion to manuscripts on information fusion", readme, re.I):
        meta_hits.append("README.md")
    log("metadata_scan.txt", "\n".join(meta_hits) if meta_hits else "CLEAN\n")
    results["metadata"] = "PASS" if not meta_hits else f"FAIL {meta_hits}"
    if meta_hits:
        failed = True

    # 11. Licence present
    results["license"] = "PASS" if (ROOT / "LICENSE").is_file() else "FAIL"
    if results["license"] != "PASS":
        failed = True

    # 12. Missing mandatory files
    mandatory = [
        "README.md",
        "LICENSE",
        "CITATION.cff",
        "zenodo.json",
        "NOTICE_DATA.md",
        "scripts/generate_assets.py",
        "scripts/verify_checksums.py",
        "src/fightsafe_ai/evaluation/aggregation_schemes.py",
        "configs/risk_fusion.yaml",
        "configs/risk_rules.yaml",
        "results/run_20260730_005150/experiment_summary.csv",
        "checksums/SHA256SUMS",
    ]
    missing = [m for m in mandatory if not (ROOT / m).is_file()]
    log("missing_files.txt", "\n".join(missing) if missing else "NONE\n")
    results["missing_files"] = "PASS" if not missing else f"FAIL {missing}"
    if missing:
        failed = True

    # 13. Manuscript compile (best-effort)
    if shutil.which("latexmk"):
        ms = subprocess.run(
            ["latexmk", "-pdf", "-interaction=nonstopmode", "main.tex"],
            cwd=ROOT / "manuscript",
            capture_output=True,
            text=True,
        )
        log("manuscript_build.txt", ms.stdout[-4000:] + "\n" + ms.stderr[-2000:])
        results["manuscript_build"] = "PASS" if ms.returncode == 0 else "FAIL"
        if ms.returncode != 0:
            failed = True
        # Appendix is inside main.tex — supplementary is not separate
        results["supplementary_build"] = results["manuscript_build"]
    else:
        results["manuscript_build"] = "SKIP (latexmk not found)"
        results["supplementary_build"] = "SKIP"

    # features_cache must not be present
    fc = list((ROOT / "optional_tier_b/inputs/features_cache").glob("*.pkl"))
    results["features_cache_absent"] = "PASS" if not fc else "FAIL"
    if fc:
        failed = True

    results["overall"] = "FAIL" if failed else "PASS"
    log("summary.json", json.dumps(results, indent=2) + "\n")
    print(json.dumps(results, indent=2))
    return 1 if failed else 0


if __name__ == "__main__":
    raise SystemExit(main())
