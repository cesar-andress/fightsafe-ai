# jss2026 reproducibility guide

Documentation for the *Journal of Systems and Software* traceability-architecture manuscript. Documentation-only; no software or metric changes beyond what **v0.1.3** already ships.

Paper-specific instructions for reproducing the **machine-side traceability stress test** reported in:

**A Formal Traceability Architecture and Auditable Human-Oversight Specification for Machine-Generated Safety Alerts**

LaTeX sources: `../jss2026/` (monorepo layout: `papers/fightsafe-ai/jss2026/`).
...
Script and directory names (`reproduce_eswa2026.sh`, `data/repro/eswa2026/`) reflect the **project-internal paper folder name at artifact creation**.

---

## Scope

Reproduce the **`jedi_submissions` machine-side stress test**: detector exports, protocol-defined evaluator bookkeeping, and manuscript Tables I–II (`tapko_pilot_results`, `tapko_pilot_per_class`).

This is **export traceability and queue-stress bookkeeping**, not fusion-accuracy validation, operator-outcome evaluation, or deployment readiness. Draft reference intervals are research bookkeeping only (see [`data/README.md`](../data/README.md)).

**Not logged in the reported run:** reviewer decisions (`H_{t,k}`), gate outcomes (`O_{t,k}`), confirmation-gate execution on footage.

**Evaluator matching defaults** (fixed in the reported run; also used by `scripts/reproduce_eswa2026.sh`):
...
|----------|---------|-------|
| `ESWA_DIR` | `../eswa2026` | Set to `../jss2026` for the active JSS manuscript tree |
...
```bash
ESWA_DIR=../jss2026 make reproduce-eswa
```
...
4. `python scripts/export_eswa2026_tables.py --install --eswa-dir ../jss2026`
5. Optional: compile `../jss2026/main.pdf` when `pdflatex` is available
...
- **Single-clip stress test:** documents export bookkeeping under partial observability, not operational deployment readiness.

---

## See also

- [`README.md`](../README.md) — installation and jss2026 quick reference
- [`JSS2026_TRACEABILITY_MATRIX.md`](JSS2026_TRACEABILITY_MATRIX.md) — manuscript-to-artifact map
